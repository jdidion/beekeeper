# Developing beekeeper

## Getting Started

If you already have the Rust toolchain installed, make sure you have at least version `1.85` (e.g., with `rustup show` or `rustc --version`).
...
When a particular topic is not covered by organizational best practices, you can [start a Discussion](https://github.com/jdidion/beekeeper/discussions) or search online (e.g., [Idiomatic Rust](https://github.com/mre/idiomatic-rust)).
...
* Creates a new [GitHub release](https://github.com/jdidion/beekeeper/releases) with changes from the (automatically updated) changelog.{% else %}

The creation of the version tag triggers the [`release`](.github/workflows/release.yml) workflow, which does the following:

* Creates a new [GitHub release](https://github.com/jdidion/beekeeper/releases) with changes from the (automatically updated) changelog.
