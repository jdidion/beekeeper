...
pub use channel::LocalQueuesImpl as ChannelLocalQueues;
#[cfg(not(any(feature = "batching", feature = "retry")))]
pub use null::LocalQueuesImpl as ChannelLocalQueues;
...
mod channel {
    ...
    pub struct LocalQueuesImpl<W: Worker> {
    ...
    #[cfg(feature = "retry")]
    impl<W: Worker> LocalQueuesImpl<W> {
        #[inline]
    ...
    impl<W: Worker> LocalQueuesImpl<W> {
        ...
        const POP_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(1);
    ...
    impl<W: Worker> LocalQueues<W, ChannelGlobalQueue<W>> for LocalQueuesImpl<W> {
        fn init_for_threads<Q: Queen<Kind = W>>(
            ...
            start_index: usize,
            end_index: usize,
            #[allow(unused_variables)] shared: &Shared<W, Q, ChannelGlobalQueue<W>, Self>,
        ) {
            #[cfg(feature = "batching")]
    ...
    mod batching {
        ...
        use crate::bee::{Queen, Worker};
        use crate::hive::{ChannelGlobalQueue, Shared, Task};
        ...
        impl<W: Worker> LocalQueuesImpl<W> {
            pub(super) fn init_batch_queues_for_threads<Q: Queen<Kind = W>>(
                ...
                (start_index..end_index)
                    .for_each(|_| batch_queues.push(ArrayQueue::new(queue_size)));
            }
    ...
    mod retry {
        use super::LocalQueuesImpl;
        use crate::bee::{Queen, Worker};
        use crate::hive::task::delay::DelayQueue;
        use crate::hive::{ChannelGlobalQueue, Shared, Task};
        use std::time::{Duration, Instant};

        impl<W: Worker> LocalQueuesImpl<W> {
            /// Initializes the retry queues worker threads in the specified range.
            pub(super) fn init_retry_queues_for_threads(
                &self,
                start_index: usize,
                end_index: usize,
            ) {
